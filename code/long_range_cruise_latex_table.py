...
def create_json_with_opt_fly_params(file_name):
    H_array = np.arange(9000, 13000, 500)
    mass_array = np.array(
...
def format_latex_table(latex_table):
    latex_table = re.sub(r'(\\bottomrule)|(\\midrule)|(\\toprule)', r'\\hline', latex_table)
    ...
    header = '\multicolumn{2}{|c|}{$m$, тонн}& \multicolumn{%s}{c|}{$H$, м}\\\ \n \cline{3-%s}\n' % (str(number_of_column-2), str(number_of_column ))
    ...
    formated_latex_table += header
    formated_latex_table += header_with_H 
